let handler = async (m, { conn, args, usedPrefix, owner }) => {
    if (args.length < 3) {
        return conn.reply(m.chat, `Gunakan format ${usedPrefix}gift <type> <jumlah> <@tag>\ncontoh penggunaan: *${usedPrefix}transfer money 100 @tag*`.trim(), m)
    } else try {
...
case 'cupon':
                if (global.db.data.users[m.sender].cupon >= count * 1) {
                    try {
                       ...
                       // conn.reply(who, `Selamat mendapatkan cupon sebesar ${count}\nFrom: *Owner*`.trim(), m)
                    } catch (e) {
                        global.db.data.users[m.sender].cupon += count * 1
                        m.reply('Gagal Menstransfer')
                        console.log(e)
                ...
                } else conn.reply(m.chat, `cupon kamu tidak mencukupi untuk mentransfer cupon sebesar ${count}`.trim(), m)
                break
