...
let handler = async (m, { conn, text }) => {
  ...
  try {
    ...
    let result = getFetchingTime(oldTime);
...
// Membuat fungsi CommonJavaScript yang mengembalikan string dengan waktu fetching
function getFetchingTime(old) {
  // Menghitung selisih waktu antara sekarang dan waktu lama dalam milidetik
  let diff = (new Date() - old) * 1;
  // Mengembalikan string dengan format yang diinginkan
  return `🍟 *Fetching* : ${diff} ms`;
}
