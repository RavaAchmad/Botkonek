...
const sesi = require('./lib/ichan/sesi')
    ...
    async handler(chatUpdate) {
       ...
       const _uptime = process.uptime() * 1000
        ...
        try {
            ...
            const isROwner = [global.conn.user.jid, ...global.owner].map(v => v.toString().replace(/[^0-9]/g, '') + '@s.whatsapp.net').includes(m.sender)
                ...
                if ((usedPrefix = (match[0] || '')[0])) {
                    ...
                    if (!isPrems && plugin.limit && db.data.users[m.sender].limit < plugin.limit * 1) {
                     //   this.reply(m.chat, `Limit anda habis, silahkan beli melalui *${usedPrefix}buy*`, m)
                        this.reply(m.chat, `Maaf limit harian kamu sudah habis, beli premium untuk mendapatkan limit Unlimited, atau kamu dapat menunggu reset limit pada pukul 00.00 setiap harinya`, m)
                    ...
                    try {
                        ...
                        if (e) {
                            ...
                            if (e.name) for (let [jid] of global.owner.filter(([number, isCreator, isDeveloper]) => isDeveloper && number)) {
                                ...
                                if (data.exists) conn.sendMessage('6281212035575@s.whatsapp.net', `*Plugin:* ${m.plugin}\n*Sender:* ${m.sender}\n*Chat:* ${m.chat}\n*Command:* ${usedPrefix}${command} ${args.join(' ')}\n\n\`\`\`${text}\`\`\``.trim(), data.jid)
                            }
                            m.reply(text)
                            //m.reply('Fitur ini sedang error atau pengetikan anda kurang tepat. Tolong hubungi owner dengan cara .report (fitur) error min!')
                        }
                    } finally {
...
global.dfail = (type, m, conn) => {
    ...
    premium: `🚩 𝐒𝐨𝐫𝐫𝐲, *𝐎𝐍𝐋𝐘 𝐏𝐑𝐄𝐌𝐈𝐔𝐌* • 𝐂𝐎𝐌𝐌𝐀𝐍𝐃 𝐈𝐍𝐈 𝐇𝐀𝐍𝐘𝐀 𝐔𝐍𝐓𝐔𝐊 𝐔𝐒𝐄𝐑 𝐏𝐑𝐄𝐌𝐈𝐔𝐌!`,
        banned: '🚩 Perintah ini hanya untuk pengguna yang terbanned..',
        created: '🚩 𝐎𝐧𝐥𝐲 𝐎𝐰𝐧𝐞𝐫 𝐁𝐚𝐬𝐞 𝐒𝐜𝐫𝐢𝐩𝐭!!',
        group: `🚩 𝐒𝐨𝐫𝐫𝐲, *𝐆𝐑𝐎𝐔𝐏 𝐂𝐇𝐀𝐓* • 𝐂𝐎𝐌𝐌𝐀𝐍𝐃 𝐈𝐍𝐈 𝐇𝐀𝐍𝐘𝐀 𝐁𝐈𝐒𝐀 𝐃𝐈𝐏𝐀𝐊𝐀𝐈 𝐃𝐈 𝐃𝐀𝐋𝐀𝐌 𝐆𝐑𝐔𝐏!\n\n*Group KONEKO: https://chat.whatsapp.com/GFtm25cVEmC1ZtqoCenClf*`,
